...
-- first name, last_name, job_id 조회
SELECT
	FIRST_NAME,
	LAST_NAME,
	JOB_ID 
...
-- 연봉이 12000 이상 되느 직원들의 LAST_NAME과 SALARY 조회
SELECT LAST_NAME, SALARY
FROM EMPLOYEES e 
WHERE SALARY >= 12000;


-- 연봉이 5000에서 12000 의 범위 이외인 사람들의 LAST_NAME과 SALARY 조회
...
-- SALARY가 2500, 3500, 7000 이 아니며 JOB 이 SA_REP 나 ST_CLERK 사원 조회
SELECT *
FROM EMPLOYEES e 
...
-- 부서 80의 각 사원에 대해 적용가능한 세율을 표시
-- 월 급여 0~ 1,999 / 세율 0%
-- 월 급여 2,000~ 3,999 / 세율 9%
-- 월 급여 4,000~ 5,999 / 세율 20%
-- 월 급여 6,000 ~7,999 / 세율 30%
-- 월 급여 8,000 ~ 9,999 / 세율 40%
-- 월 급여 10,000 ~ 11,999 / 세율 42%
-- 월 급여 12,000 ~ 13,999 / 세율 44%
-- 월 급여 14,000~ / 세율 45%
...
SELECT LAST_NAME, SALARY,
	CASE
	WHEN TRUNC(SALARY/2000,0) < 0 THEN '0%'
	WHEN TRUNC(SALARY/2000,0) < 1 THEN '1%'
	WHEN TRUNC(SALARY/2000,0) < 2 THEN '2%'
	WHEN TRUNC(SALARY/2000,0) < 3 THEN '3%'
	WHEN TRUNC(SALARY/2000,0) < 4 THEN '40%'
	WHEN TRUNC(SALARY/2000,0) < 5 THEN '42%'
	WHEN TRUNC(SALARY/2000,0) < 6 THEN '44%'
	ELSE '45%'
END AS TAX_RATE
FROM EMPLOYEES e
WHERE DEPARTMENT_ID = 80;
...
-- 매니저가 관리하는 사원들 중에서 최소급여가 6000미만인 최소급여는 제외
-- 매니저가 없는 사원 제외 !! 
...
-- 사원들의 이름 조회(이름은 FIRST_NAME과 LAST_NAME을 결합하여 하나로 나오게하기)
...
-- 기존의 직업을 여전히 가지고 있는 사원들의 사번 및 JOB_ID 조회 (서브 쿼리 사용)
...
SELECT JOB_ID, (SELECT SUM(SALARY) FROM EMPLOYEES e2 WHERE DEPARTMENT_ID  = 10 AND E.JOB_ID =E2.JOB_ID) AS DEPR10,
(SELECT SUM(SALARY) FROM EMPLOYEES e2 WHERE DEPARTMENT_ID  = 20 AND E.JOB_ID =E2.JOB_ID) AS DEPR20,
(SELECT SUM(SALARY) FROM EMPLOYEES e2 WHERE DEPARTMENT_ID  = 30 AND E.JOB_ID =E2.JOB_ID) AS DEPR30,
(SELECT SUM(SALARY) FROM EMPLOYEES e2 WHERE DEPARTMENT_ID  = 40 AND E.JOB_ID =E2.JOB_ID) AS DEPR40,
(SELECT SUM(SALARY) FROM EMPLOYEES e2 WHERE DEPARTMENT_ID  = 50 AND E.JOB_ID =E2.JOB_ID) AS DEPR50,
(SELECT SUM(SALARY) FROM EMPLOYEES e2 WHERE DEPARTMENT_ID  = 60 AND E.JOB_ID =E2.JOB_ID) AS DEPR60,
(SELECT SUM(SALARY) FROM EMPLOYEES e2 WHERE DEPARTMENT_ID  = 70 AND E.JOB_ID =E2.JOB_ID) AS DEPR70,
(SELECT SUM(SALARY) FROM EMPLOYEES e2 WHERE DEPARTMENT_ID  = 80 AND E.JOB_ID =E2.JOB_ID) AS DEPR80,
(SELECT SUM(SALARY) FROM EMPLOYEES e2 WHERE DEPARTMENT_ID  = 90 AND E.JOB_ID =E2.JOB_ID) AS DEPR90,
(SELECT SUM(SALARY) FROM EMPLOYEES e2 WHERE DEPARTMENT_ID  = 100 AND E.JOB_ID =E2.JOB_ID) AS DEPR100,
(SELECT SUM(SALARY) FROM EMPLOYEES e2 WHERE DEPARTMENT_ID  = 110 AND E.JOB_ID =E2.JOB_ID) AS DEPR110,
...
FROM EMPLOYEES e 
WHERE SALARY > (SELECT MAX(SALARY) FROM EMPLOYEES e WHERE e.JOB_ID = 'SA_MAN'); 


-- 커미션을 버는 사원들의 부서와 연봉이 동일한 사원들의 LAST_NAME, 부서 번호, 연봉 조회

SELECT LAST_NAME, DEPARTMENT_ID , SALARY
FROM EMPLOYEES e WHERE (DEPARTMENT_ID, SALARY) IN
(SELECT DEPARTMENT_ID, SALARY FROM EMPLOYEES e WHERE COMMISSION_PCT IS NOT NULL)


-- 회사 전체 평균연봉보다 더 많이 버는 사원들 중 LAST_NAME에 u가 있는 사원들이 근무하는 부서에서 
-- 근무하는 사원들의 사번, LAST_NAME, 연봉 조회

SELECT EMPLOYEE_ID, LAST_NAME, SALARY 
FROM EMPLOYEES e 
WHERE DEPARTMENT_ID IN
(SELECT DEPARTMENT_ID FROM EMPLOYEES e WHERE SALARY > (SELECT AVG(SALARY) FROM EMPLOYEES e) AND LAST_NAME LIKE '%u%');



-- 각 부서별 평균 연봉보다 더 받는 동일 부서 근무사원들의 
-- LAST_NAME, 부서 번호, 연봉 및 해당 부서의 평균 연봉 조회
-- 부서별 연봉을 기준으로 정렬
SELECT
	E1.LAST_NAME,
	E1.DEPARTMENT_ID,
...
-- LAST_NAME이 Davies 인 사람보다 후에 고용된 사원들의 LAST_NAME, HITR_DATE 조회
SELECT LAST_NAME, HIRE_DATE 
FROM EMPLOYEES e 
WHERE HIRE_DATE > (SELECT HIRE_DATE FROM EMPLOYEES e WHERE LAST_NAME LIKE 'Davies');
...
FROM EMPLOYEES e 
WHERE MANAGER_ID IN
(SELECT EMPLOYEE_ID FROM EMPLOYEES e WHERE LAST_NAME = 'king');


-- LAST_NAME 이 Kochher 인 사원과 동일한 연봉 및 커미션을 버는 사원들의 LAST_NAME, 부서 번호, SALARY 조회
-- Kochhar 은 제외한다. (결과 안나옴)
SELECT LAST_NAME, DEPARTMENT_ID, SALARY 
FROM EMPLOYEES e 
WHERE (SALARY, NVL(COMMISSION_PCT, 0)) IN (SELECT SALARY, NVL(COMMISSION_PCT, 0) FROM EMPLOYEES e WHERE LAST_NAME = 'kochhar');
...
(SELECT L.COUNTRY_ID FROM LOCATIONS l ,
	(SELECT LOCATION_ID FROM DEPARTMENTS d) D
...
-- 위치ID 가 1700인 사원들의 연봉과 커미션을 추출한 뒤, 추출된 사원들의 연봉과 커미션이 동일한 사원정보 조회
